...
res = torch.tensor(res, dtype=torch.float16, requires_grad=True).to(device)
b_left = torch.tensor(b_left, dtype=torch.float16, requires_grad=True).to(device)
b_right = torch.tensor(b_right, dtype=torch.float16, requires_grad=True).to(device)
b_upper = torch.tensor(b_upper, dtype=torch.float16, requires_grad=True).to(device)
b_lower = torch.tensor(b_lower, dtype=torch.float16, requires_grad=True).to(device)
...
if args.model == 'KAN':
    model = get_model(args).Model(width=[2, 5, 5, 1], grid=5, k=3, grid_eps=1.0, \
                                  noise_scale_base=0.25, device=device).to(torch.float16).to(device)
elif args.model == 'QRes':
    model = get_model(args).Model(in_dim=2, hidden_dim=256, out_dim=1, num_layer=4).to(torch.float16).to(device)
    model.apply(init_weights)
elif args.model == 'PINNsFormer' or args.model == 'PINNsFormer_Enc_Only':
    model = get_model(args).Model(in_dim=2, hidden_dim=32, out_dim=1, num_layer=1).to(torch.float16).to(device)
    model.apply(init_weights)
else:
    model = get_model(args).Model(in_dim=2, hidden_dim=512, out_dim=1, num_layer=4).to(torch.float16).to(device)
    model.apply(init_weights)
...
loss_track = []
pi = torch.tensor(np.pi, dtype=torch.float16, requires_grad=False).to(device)
...
flops_log_file_path = f'./results/wave/1dwave_{args.model}_flops_log.txt'
...
for i in tqdm(range(10000)):
    ...
    # Calculate and store losses after the optimizer step (only once per epoch)
    # Re-compute losses for logging (using the model's current state)
    pred_res = model(x_res, t_res)
    pred_left = model(x_left, t_left)
    pred_upper = model(x_upper, t_upper)
    pred_lower = model(x_lower, t_lower)
    ...
    u_t = torch.autograd.grad(pred_res, t_res, grad_outputs=torch.ones_like(pred_res), retain_graph=True,
                              create_graph=True)[0]
    u_tt = torch.autograd.grad(u_t, t_res, grad_outputs=torch.ones_like(pred_res), retain_graph=True,
                               create_graph=True)[0]
    ...
    # Plus 5 gradient computations: u_x, u_xx, u_t, u_tt, ui_t
    num_forward_passes = 5
    num_grad_computations = 5
...
res_test = torch.tensor(res_test, dtype=torch.float16, requires_grad=True).to(device)
...
try:
    ...
    plt.tight_layout()
    plt.savefig(f'./results/wave/1dwave_{args.model}_flops_curve.pdf', bbox_inches='tight')
    plt.close()
    print(f'FLOPs/FLOPS curve saved to: ./results/wave/1dwave_{args.model}_flops_curve.pdf')
