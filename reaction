...
# ========== CALCULATE FLOPs ONCE (NOT EVERY EPOCH) ==========
...
total_flops_per_epoch = total_forward_flops + total_backward_flops
# ============================================================

print(f"FLOPs calculated once before training:")
print(f"  Forward FLOPs per epoch: {total_forward_flops:.2e}")
print(f"  Backward FLOPs per epoch: {total_backward_flops:.2e}")
...
for i in tqdm(range(2000)): # epoch changed from 2000 to 500
    ...
    # ========== USE PRE-CALCULATED FLOPs (NOT RECALCULATING) ==========
    ...
    # Calculate FLOPS (FLOPs per second) using pre-calculated total_flops_per_epoch
    if total_time > 0:
        flops_per_sec = total_flops_per_epoch / total_time
    else:
        flops_per_sec = 0.0
    
    flops_track.append([total_forward_flops, total_backward_flops, total_flops_per_epoch, forward_time_measured, backward_time_measured, total_time, flops_per_sec])
    # ====================================================================
    ...
    # Log FLOPs/FLOPS to file after each epoch (using pre-calculated values)
    with open(flops_log_file_path, 'a') as flops_log_file:
        flops_log_file.write(f'{i+1},{total_forward_flops:.2e},{total_backward_flops:.2e},{total_flops_per_epoch:.2e},'
