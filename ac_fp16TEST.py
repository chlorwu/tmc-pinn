...
for iter in tqdm(range(1)):
    ...
    flops_log_file_path = f'./results/1dac_{args.model}_flops_log.txt'
    ...
    for i in tqdm(range(10000)): 
        ...
        # Plus 5 gradient computations (u_x, u_xx, u_t, bnd_upper, bnd_lower)
        ...
        with open(flops_log_file_path, 'a') as flops_log_file:
            flops_log_file.write(f'{i},{total_forward_flops:.2e},{total_backward_flops:.2e},{total_flops:.2e},'
    ...
    try:
        ...
        plt.tight_layout()
        plt.savefig(f'./results/1dac_{args.model}_flops_curve.pdf', bbox_inches='tight')
        plt.close()
        
        print(f'FLOPs/FLOPS curve saved to: ./results/1dac_{args.model}_flops_curve.pdf')
